...
def plotting(
    fs, template_p, template_c, time, template_offset, windows, mlab, whiten,
    interp1d, dt, bb, ab, normalization, make_plots,
    strain_L1, strain_H1, strain_L1_whitenbp, strain_H1_whitenbp,
    eventname, plottype, FIG_DIR, tevent, plt, np, filtfilt):
    ...
    for det in dets:
        ...
        if make_plots:
            ...
            plt.xlabel('Time since {0:.4f}'.format(timemax))
            plt.legend(loc='upper left')
            plt.savefig(FIGURES / f"{eventname}_{det}_SNR.{plottype}")
            ...
            plt.savefig(FIGURES / f"{eventname}_{det}_matchtime.{plottype}")
            ...
            plt.savefig(FIGURES / f"{eventname}_{det}_matchfreq.{plottype}")
