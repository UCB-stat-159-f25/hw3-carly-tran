...
def test_loaddata_runs_and_shapes_match():
    """Basic sanity for loaddata: arrays exist, lengths match, time monotonic, fs>0."""
    ...
    # time should be strictly increasing (or at least non-decreasing)
    dt = np.diff(time)
    assert np.all(dt > 0) or np.all(dt >= 0)

    # estimate sampling rate from time differences
    # (robust to tiny jitter: use median)
    fs_est = 1.0 / np.median(dt)
    assert np.isfinite(fs_est) and fs_est > 0

    # If metadata *does* include fs, check consistency loosely
